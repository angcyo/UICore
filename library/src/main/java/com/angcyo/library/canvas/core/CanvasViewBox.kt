 ...
 * @since 2023/06/28
 */
open class CanvasViewBox(val iCanvasView: ICanvasView) {
    ...
    fun updateRenderBounds(newBounds: RectF) {
        ...
        iCanvasView.dispatchRenderBoxBoundsUpdate(newBounds)
    ...
    fun updateOriginGravity(gravity: Int) {
        ...
        iCanvasView.dispatchRenderBoxOriginGravityUpdate(gravity)
    ...
    fun updateRenderMatrix(target: Matrix, finish: Boolean, reason: Reason) {
        ...
        iCanvasView.dispatchRenderBoxMatrixUpdate(target, reason, finish)
    ...
    fun changeRenderMatrix(
        ...
        if (anim) {
            _changeAnimator = matrixAnimator(from, target, finish = { isCancel ->
                ...
                iCanvasView.dispatchRenderBoxMatrixChange(from, target, reason)
            }) {
    ...
    /**增量平移画布*/
    open fun translateBy(dx: Float, dy: Float, anim: Boolean = true, reason: Reason = Reason.user) {
        val matrix = Matrix(renderMatrix)
        matrix.postTranslate(dx, dy)
        changeRenderMatrix(matrix, anim, reason)
    }

    /**直接平移画布*/
    open fun translateTo(tx: Float, ty: Float, anim: Boolean = true, reason: Reason = Reason.user) {
        val matrix = Matrix(renderMatrix)
    ...
    /**增量缩放画布*/
    open fun scaleBy(sx: Float, sy: Float, anim: Boolean = true, reason: Reason = Reason.user) {
        //默认用画板的中点进行缩放
    ...
    /**直接缩放画布*/
    open fun scaleTo(sx: Float, sy: Float, anim: Boolean = true, reason: Reason = Reason.user) {
        val matrix = Matrix(renderMatrix)
