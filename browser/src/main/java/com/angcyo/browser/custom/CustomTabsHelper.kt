...
object CustomTabsHelper {
    ...
    const val EDGE_STABLE_PACKAGE = "com.microsoft.emmx"
    ...
    fun getPackageNameToUse(context: Context): String? {
        ...
        if (packagesSupportingCustomTabs.isEmpty()) {
            activityIntent.setPackage(EDGE_STABLE_PACKAGE)
            val list = pm.queryIntentActivities(activityIntent, 0)
            if (list.isNotEmpty()) {
                packagesSupportingCustomTabs.add(EDGE_STABLE_PACKAGE)
            }
        }

        //chrome
        if (packagesSupportingCustomTabs.isEmpty()) {
            activityIntent.setPackage(STABLE_PACKAGE)
            val list = pm.queryIntentActivities(activityIntent, 0)
            if (list.isNotEmpty()) {
                packagesSupportingCustomTabs.add(STABLE_PACKAGE)
            }
        }
